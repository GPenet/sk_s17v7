...
next9: // add clue 9 build td and call next step (min 10 clues)
	{
		...
		if (op.known ) {
			...
			if (!((~pk54) & sp9.all_previous_cells)) {
				cout << Char54out(sp9.all_previous_cells) << " expected 9 " << endl;
				knownt = 9;
				if (op.ton > 1) {	t54b12.DebugC();	sp9.v.Print(" v ");	}
			}
		}
		if (locdiag)cout << Char54out(sp9.all_previous_cells) << "  9 " << endl;
...
void G17B::Valid9(SPB03A& s9) {// can be any process
	...
	p_cpt2g[56]++;
#ifdef  T18P2
	cout << Char54out(bf_cl9) << "valid 9" << endl;
#endif
	clean_valid_done = 1;
	DoBuild9();
	int nc12 = 9, nc3 = 8;
...
void G17B::Go_x_11_18() {// 8 clues limit 11 clues 
	...
	while (bitscanforward64(tclx[2], Ac2)) {
		...
		while (bitscanforward64(tclx[3], Ac3)) {
			...
			myb12 = bf3; ncluesb12 = 10; ncluesb3 = 8;
			...
			while (bitscanforward64(tclx[4], Ac4)) {
				...
				myb12 = bf4; ncluesb12 = 11; ncluesb3 = 7;
		...
		while (bitscanforward64(tclx[3], Ac3)) {
			...
			while (bitscanforward64(tclx[4], Ac4)) {
				...
				while (bitscanforward64(tclx[5], Ac5)) {
					uint64_t bit5 = (uint64_t)1 << tclx[4];
					Ac4 ^= bit5; //clear bit
					myb12 = bf4 | bit5;		// 12 6
					ncluesb12 = 12; ncluesb3 = 6;
...
void G17B::GoCallB3Com() {
	...
	if (locdiag) {
		...
		//guah54n.Status(2);
		guah54n.zz[0].Dump();
		guah54n.zz[0].v0.Print("v0");
		guah54n.zz[0].v6.Print("v6");
		guah54n.zz[0].v9.Print("v9");
		guah54n.g2.Print("g2");
		guah54n.g3.Print("g3");
	}
	tcluesxpdone = 0;
...
//			if(xq.MissxToMiss0(xq.Isoutsize2())) return;
	...
	if (xq.nmiss == 4) {
		int x = xq.Isoutsize3();
		if (locdiag) {
			cout << Char27out(x) << "x size 3=" << endl;
		}
		if (!x) {//go direct
			xq.BuildMissxOut();
			g17b.GoEndAll(0, BIT_SET_27);
					...
					if (cc == 4) {
						...
						if (op.dv3) {
							...
							cout << Char27out(w.bf.u32[2]) << " added >4 " 
								<< " [8] " << p_cpt2g[8] << myband3->nbgmm << endl;
						}
					}
					continue;
				}
...
int XQ::Isoutsize4() {
	if (nout < 4) return 0;
	uint32_t n = nout; if (n > 30) n = 30;
	uint32_t limi1 = n - 3; if (limi1 > 12)limi1 = 12;
